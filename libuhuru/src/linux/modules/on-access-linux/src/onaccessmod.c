...
static enum uhuru_mod_status mod_oal_conf_modules(struct uhuru_module *module, const char *key, struct uhuru_conf_value *value)
{
  struct uhuru_scan_conf *on_access_conf = uhuru_scan_conf_on_access();
...
static enum uhuru_mod_status mod_oal_conf_mime_types(struct uhuru_module *module, const char *key, struct uhuru_conf_value *value)
{
  struct uhuru_scan_conf *on_access_conf = uhuru_scan_conf_on_access();

  if (uhuru_conf_value_is_string(value))
    uhuru_scan_conf_add_mime_type(on_access_conf, uhuru_conf_value_get_string(value));
  else {
    const char **p;

    for (p = uhuru_conf_value_get_list(value); *p != NULL; p++)
      uhuru_scan_conf_add_mime_type(on_access_conf, *p);
  }
