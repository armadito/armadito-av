...
JRPC_STRUCT(a6o_base_info)
	JRPC_FIELD_STRING(name)
	JRPC_FIELD_INT(time_t, base_update_ts)
	JRPC_FIELD_STRING(version)
	JRPC_FIELD_INT(size_t, signature_count)
	JRPC_FIELD_STRING(full_path)
JRPC_END_STRUCT

JRPC_STRUCT(a6o_module_info)
	JRPC_FIELD_STRING(name)
	JRPC_FIELD_ENUM(a6o_update_status, mod_status)
	JRPC_FIELD_INT(time_t, mod_update_ts)
	JRPC_FIELD_ARRAY(a6o_base_info, base_infos)
JRPC_END_STRUCT
