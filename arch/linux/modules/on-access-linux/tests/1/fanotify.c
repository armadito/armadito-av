...
struct watchd *watchd_new(void)
{
	...
	if (w->fanotify_fd < 0)
		a6o_log(A6O_LOG_MODULE, A6O_LOG_LEVEL_ERROR, "fanotify init failed (%s)", strerror(errno));
	...
	a6o_log(A6O_LOG_MODULE, A6O_LOG_LEVEL_DEBUG, "fanotify init ok");
...
void watchd_add(struct watchd *w, const char *path)
{
	...
	if (fanotify_mark(w->fanotify_fd, FAN_MARK_ADD | FAN_MARK_MOUNT, FAN_OPEN_PERM, AT_FDCWD, tmp) < 0)
		a6o_log(A6O_LOG_MODULE, A6O_LOG_LEVEL_ERROR, "fanotify mark on %s failed (%s)", tmp, strerror(errno));

	g_ptr_array_add(w->paths, (gpointer)tmp);

	a6o_log(A6O_LOG_MODULE, A6O_LOG_LEVEL_DEBUG, "fanotify marked %s", tmp);
}
...
static void event_process(struct watchd *w, struct fanotify_event_metadata *event)
{
	...
	a6o_log(A6O_LOG_MODULE, A6O_LOG_LEVEL_DEBUG, "received event fd %d path '%s'", event->fd, p ? p : "unknown");
...
static gboolean watchd_cb(GIOChannel *source, GIOCondition condition, gpointer data)
{
	...
	a6o_log(A6O_LOG_MODULE, A6O_LOG_LEVEL_DEBUG, "fanotify callback");
...
void watchd_free(struct watchd *w)
{
	...
	a6o_log(A6O_LOG_MODULE, A6O_LOG_LEVEL_DEBUG, "fanotify free ok");
}
